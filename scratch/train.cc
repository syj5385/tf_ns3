...
class Model {
 public:
  ...
  void Init() { TF_CHECK_OK(session->Run({}, {}, {"init"}, nullptr)); }
  ...
  void Predict(const std::vector<float>& batch) {
    ...
    TF_CHECK_OK(session->Run({{"input", MakeTensor(batch)}}, {"output"}, {},
                              &out_tensors));
  ...
  void RunTrainStep(const std::vector<float>& input_batch,
    ...
    TF_CHECK_OK(session->Run({{"input", MakeTensor(input_batch)},
...
int main(int argc, char* argv[]) {
  const string graph_def_filename = "./tensorflow_model/graph2.pb";
  const string checkpoint_dir = "./checkpoint";
